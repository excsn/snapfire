...
| Keep a session alive past `session.ttl` | `session.extend(seconds)` in an action or middleware or `ctx.session.extend` from Rust code that has the request; nothing extends on its own |
