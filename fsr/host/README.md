...
| Keep a session alive past `session.ttl` | `session.extend(seconds)` in an action, a route handler or middleware or `ctx.session.extend` from Rust code that has the request; nothing extends on its own |
...
| Try a policy before enforcing it | `[document.csp_report_only]`, the same shape, sent as report-only |
