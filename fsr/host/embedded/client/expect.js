...
function isAsymmetric(value) {
    return typeof value === "object" && value !== null && value.$$typeof === ASYMMETRIC;
}
function asymmetric(text, match) {
    return {
        $$typeof: ASYMMETRIC,
...
export function equals(a, b, strict = false, seen = []) {
    ...
    if (isNode(a) || isNode(b)) return isNode(a) && isNode(b) && (a === b || typeof a.isEqualNode === "function" && a.isEqualNode(b));
    ...
    return ka.every((k)=>Object.prototype.hasOwnProperty.call(b, k) && equals(a[k], b[k], strict, seen));
}
...
export const SETTLED = Symbol.for("sf.settled");
function marked(promise, settled) {
...
function resolved(value) {
    return marked(Promise.resolve(value), {
        ok: true,
        value
    });
}
...
export function fn(impl) {
    ...
    Object.assign(mock, {
        ...
        mockResolvedValue (value) {
            implementation = ()=>resolved(value);
            return mock;
        },
        mockResolvedValueOnce (value) {
            once.push(()=>resolved(value));
            return mock;
        },
        mockRejectedValue (error) {
            implementation = ()=>rejected(error);
            return mock;
        },
        mockRejectedValueOnce (error) {
            once.push(()=>rejected(error));
            return mock;
        },
        mockReturnThis () {
...
export function spyOn(object, key, access) {
    if (access) {
...
function thrownText(error) {
    if (error instanceof Error) return `${error.kind ?? ""} ${error.message}`.trim();
...
function valueOf(el) {
    if (el.tagName === "SELECT") {
        const select = el;
        const chosen = Array.from(select.querySelectorAll("option")).filter((o)=>o.selected).map((o)=>String(o.value));
...
function formValues(form) {
    ...
    for (const [name, group] of byName){
        ...
        if (type === "checkbox") {
            out[name] = group.length === 1 && !group[0].hasAttribute("value") ? group[0].checked : group.filter((c)=>c.checked).map((c)=>c.getAttribute("value") ?? "on");
        } else if (type === "radio") {
            const on = group.find((c)=>c.checked);
...
function callsWith(mock, args) {
    return mock.mock.calls.some((call)=>equals(call, args));
}
function returns(mock) {
...
const matchers = {
    ...
    toContain (received, item) {
        ...
        else if (received !== null && received !== undefined && typeof received[Symbol.iterator] === "function") pass = Array.from(received).some((x)=>x === item || sameInteger(x, item));
        return result(pass, `a value containing ${show(item)}`, received);
    },
    toContainEqual (received, item) {
        const pass = received !== null && received !== undefined && typeof received[Symbol.iterator] === "function" && Array.from(received).some((x)=>equals(x, item));
        return result(pass, `a value containing an item equal to ${show(item)}`, received);
    },
    toHaveLength (received, length) {
        const has = received !== null && received !== undefined ? received.length : undefined;
        return result(has === length, `length ${show(length)}`, received, `Received length: ${show(has)}`);
    },
    toHaveProperty (received, path, ...value) {
        const keys = Array.isArray(path) ? path : String(path).replace(/\[(\w+)\]/g, ".$1").split(".").filter((k)=>k !== "");
        let at = received;
        let found = true;
    ...
    toThrow (received, expected) {
        let thrown = null;
        if (this.promise === "rejects") thrown = {
            value: received
        };
        else {
        ...
        return {
            pass,
            message: ()=>thrown === null ? "Received function did not throw" : `Expected: ${pass ? "not " : ""}${expected === undefined ? "to throw" : `a throw matching ${show(expected)}`}\nThrown: ${show(thrown.value)}`
        };
    },
    toBeInstanceOf (received, type) {
        return result(received instanceof type, `an instance of ${type.name ?? show(type)}`, received);
    },
    ...
    toHaveBeenNthCalledWith (received, n, ...args) {
    ...
    toHaveNthReturnedWith (received, n, value) {
    ...
    toHaveTextContent (received, text, options = {}) {
    ...
    toHaveClass (received, ...names) {
        ...
        const options = typeof names[names.length - 1] === "object" ? names.pop() : {};
    ...
    toBePartiallyChecked (received) {
        ...
        return result(checkedState(el) === "mixed" || el.indeterminate === true, "a partially checked element", el);
    },
    ...
    toHaveDisplayValue (received, value) {
        ...
        const shown = el.tagName === "SELECT" ? Array.from(el.querySelectorAll("option")).filter((o)=>o.selected).map((o)=>o.textContent ?? "") : [
            String(el.value ?? "")
        ];
    ...
    toContainElement (received, other) {
    ...
    toHaveStyle (received, css) {
        const el = requireElement(received, "toHaveStyle");
        const style = inlineStyle(el);
        const want = typeof css === "string" ? css.split(";").map((p)=>p.split(":")).filter((p)=>p.length >= 2).map(([k, ...v])=>[
                k.trim().toLowerCase(),
                v.join(":").trim()
...
function check(name, matcher, received, args, isNot, promise, message) {
    counted++;
    const state = {
        isNot,
        promise,
...
function plain(received, isNot, message) {
    const out = {};
    for (const [name, matcher] of Object.entries(matchers))out[name] = (...args)=>check(name, matcher, received, args, isNot, "", message);
    return out;
}
function settling(received, how, isNot, message) {
    const out = {};
    for (const [name, matcher] of Object.entries(matchers)){
        out[name] = async (...args)=>{
            const pending = typeof received === "function" ? received() : received;
            let outcome;
            try {
                outcome = {
                    ok: true,
...
function anyOf(type) {
    const name = type.name ?? String(type);
    return asymmetric(`Any<${name}>`, (other)=>{
        ...
        return typeof type === "function" && other instanceof type;
    });
}
const objectContaining = (expected)=>asymmetric(`ObjectContaining ${show(expected)}`, (other)=>typeof other === "object" && other !== null && Object.entries(expected).every(([k, v])=>k in other && equals(other[k], v)));
const arrayContaining = (expected)=>asymmetric(`ArrayContaining ${show(expected)}`, (other)=>Array.isArray(other) && expected.every((e)=>other.some((o)=>equals(o, e))));
const stringContaining = (expected)=>asymmetric(`StringContaining ${show(expected)}`, (other)=>typeof other === "string" && other.includes(expected));
const stringMatching = (expected)=>asymmetric(`StringMatching ${show(expected)}`, (other)=>typeof other === "string" && (typeof expected === "string" ? new RegExp(expected) : expected).test(other));
const negate = (m)=>asymmetric(`Not${m.toString()}`, (other)=>!m.asymmetricMatch(other));
export const expect = Object.assign((received, message)=>{
    const assertion = plain(received, false, message);
    assertion.not = plain(received, true, message);
    assertion.resolves = Object.assign(settling(received, "resolves", false, message), {
        not: settling(received, "resolves", true, message)
    });
    assertion.rejects = Object.assign(settling(received, "rejects", false, message), {
        not: settling(received, "rejects", true, message)
    });
    return assertion;
}, {
    any: anyOf,
    ...
    stringContaining,
    stringMatching,
    closeTo: (n, digits = 2)=>asymmetric(`NumberCloseTo ${n} (${digits} digits)`, (other)=>typeof other === "number" && Math.abs(other - n) < 10 ** -digits / 2),
    not: {
        objectContaining: (e)=>negate(objectContaining(e)),
        arrayContaining: (e)=>negate(arrayContaining(e)),
        stringContaining: (e)=>negate(stringContaining(e)),
        stringMatching: (e)=>negate(stringMatching(e))
    },
    assertions (count) {
        expectedCount = count;
    },
