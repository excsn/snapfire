...
* `intercept_for(&self, path: &str, from: Option<&str>, into: Option<&str>) -> Option<(PlanNode, Params)>`: the intercept a soft navigation to `path` renders, when the route has one: with `into`, when it names the intercept's slot; otherwise when `from`, the origin's path, matches a route whose layouts, module for module from the shell down, reach the layout declaring the slot. Both paths are without their query.
