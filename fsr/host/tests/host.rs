...
const PLAN: &str = r#"{
  ...
  "routes": [
    ...
    { "pattern": "/where", "plan": { "id": 0, "module": "shell#document", "children": [
      { "slot": "content", "node": { "id": 1, "module": "routes/where/page.tsx#default", "source": "where" } } ] } },
...
async fn a_route_renders_through_the_stock_shell_with_the_configured_head() {
  ...
  assert!(
    ...
    "the title is escaped: {html}"
  );
  assert!(
    html.contains("<script type=\"importmap\">{\"imports\":{}}</script>"),
    "{html}"
  );
  assert!(
    html.contains("<script type=\"module\" src=\"/static/app.js\"></script>"),
    "{html}"
  );
  assert!(
    html.contains("data-sf-module=\"routes/index/page.tsx#default\""),
    "{html}"
  );
...
async fn the_edge_serves_static_files_actions_and_pages_with_a_session_cookie() {
  ...
  let response = host
    .handle(Request::get("/static/app.js").body(Bytes::new()).unwrap())
    .await;
  ...
  let response = host
    .handle(Request::get("/static/missing.js").body(Bytes::new()).unwrap())
    .await;
  ...
  let cookie = response
    ...
    .expect("a fresh session sets its cookie")
    .to_str()
    .unwrap()
    .to_owned();
  let body = response.into_body().collect().await.unwrap().to_bytes();
  ...
  assert_eq!(
    ...
    "the input is checked against the contract"
  );

  let response = host
    .handle(Request::get("/hello/x?from=y&__payload").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
...
async fn over_h2(
  ...
  path: &str,
) -> Result<(http::Version, String), Box<dyn std::error::Error + Send + Sync>> {
...
async fn http2_is_off_until_the_configuration_asks_for_it() {
  ...
  assert!(
    ...
    "an http/1.1 listener refuses the preface rather than answering it"
  );
}
...
async fn http2_serves_a_prior_knowledge_client_beside_http1() {
  ...
  assert!(
    host.report().to_string().contains("h2c beside http/1.1"),
    "{}",
    host.report()
  );
  ...
  let (version, body) = tokio::time::timeout(std::time::Duration::from_secs(5), over_h2(addr, "/hello/h2?from=two"))
    .await
    .unwrap()
    .unwrap();
  ...
  assert!(
    ...
    "the same listener still answers http/1.1: {response}"
  );
...
async fn the_tower_service_answers_like_the_edge() {
  ...
  let response = host
    .service()
    .oneshot(
      Request::get("/hello/tower?from=svc")
...
async fn axum_nests_the_host_under_a_prefix() {
  ...
  let response = app
    .oneshot(
      Request::get("/shop/hello/axum?from=router")
  ...
  assert!(
    String::from_utf8_lossy(&body).contains("hi axum via router"),
    "{}",
...
fn a_project_root_with_a_config_directory_infers_the_rest_from_the_app() {
  ...
  std::fs::write(
    app.join("dist/.snapfire-build.json"),
    r#"{"version":1,"entries":["src/main.js"],"publicPath":"/static/js/app/","outputs":[],"externals":[],"graph":{}}"#,
  )
  .unwrap();
  std::fs::write(app.join("vendor/react/react.js"), "react").unwrap();
  std::fs::write(
    app.join("importmap.json"),
    r#"{"imports":{"react":"/static/js/vendor/react/react.js"}}"#,
  )
  .unwrap();
  ...
  assert!(
    report.contains("/static/js/app"),
    "dist is served at the build's publicPath: {report}"
  );
  assert!(
    report.contains("/static/js/vendor"),
    "vendor/ is served by convention: {report}"
  );
  assert!(report.contains("inferred"), "{report}");
  assert!(
    report.contains("shop"),
    "the client document was found under clients/: {report}"
  );

  let rt = tokio::runtime::Runtime::new().unwrap();
  let html = rt
    .block_on(host.render_to_string("/hello/x?from=y", RenderMode::Html, SessionCell::default()))
    .unwrap();
  assert!(
    html.contains("<title>Layered</title>"),
    "the later file in config/ wins: {html}"
  );
  assert!(
    html.contains("src=\"/static/js/app/src/main.js\""),
    "the entry is inferred: {html}"
  );
  assert!(html.contains("\"react\""), "the import map is inferred: {html}");
  assert!(
    html.contains("<link rel=\"stylesheet\" href=\"/static/css/app.css\">"),
    "the stylesheet is inferred: {html}"
  );
  ...
  let response = rt.block_on(
    host.handle(
      Request::get("/static/js/vendor/react/react.js")
        .body(Bytes::new())
        .unwrap(),
    ),
  );
...
fn a_config_directory_loads_the_deployment_ladder_in_order_and_nothing_else() {
  ...
  let deployment = Deployment {
    release_env: "staging".to_owned(),
    app_env: "local".to_owned(),
    region: Some("eu".to_owned()),
  };
  let names: Vec<String> = config_paths(&dir, &deployment)
    .iter()
...
async fn a_cache_section_installs_the_render_memo_and_the_report_says_so() {
  ...
  let host = Host::from(dir.join("app.toml"))
    ...
    .unwrap();
  assert!(
    host.report().to_string().contains("cache     1000 entries, ttl 5m"),
    "{}",
    host.report()
  );

  host
    .render_to_string("/hello/norm", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  host
    .render_to_string("/hello/norm", RenderMode::Html, SessionCell::default())
    ...
    .unwrap();
  host
    .render_to_string("/hello/ada", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert_eq!(
    host.invalidate("routes/hello/page.tsx#default").await,
    2,
    "one entry per distinct params"
  );
  assert_eq!(host.invalidate("routes/hello/page.tsx#default").await, 0);

  std::fs::write(dir.join("app.toml"), format!("{base}\n[cache]\nttl = \"soon\"\n")).unwrap();
  let Err(err) = Host::from(dir.join("app.toml"))
    .unwrap()
    .services_over(Arc::new(MockTransport::new()))
    .build()
  else {
    panic!("a lifetime nobody can parse must refuse to start")
  };
...
async fn development_documents_carry_the_refresh_script_and_the_host_announces_changes() {
  ...
  assert!(
    host
      .report()
      .to_string()
      .contains("dev       live refresh on /__fsr/events"),
    "{}",
    host.report()
  );
  let html = host
  ...
  let response = host
    .handle(Request::get("/__fsr/events").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(response.status(), StatusCode::OK);
  ...
  assert!(
    html.contains("var b=\"-\""),
    "the document carries the id it was rendered against: {html}"
  );
  let told = host
    .handle(Request::post("/__fsr/changed").body(Bytes::new()).unwrap())
    .await;
  ...
  assert!(
    with_bundle.starts_with("data: {\"bundle\":\"") && !with_bundle.contains("\"-\""),
    "a bundle that appeared has an id: {with_bundle}"
  );
  ...
  assert_eq!(
    ...
    "no-cache",
    "statics revalidate in development"
  );
}
...
async fn a_soft_navigation_is_intercepted_when_its_origin_shares_the_declaring_layout() {
  let (host, _) = host();
  assert!(
    host.intercept_for("/feed/photo/3", Some("/feed"), None).is_some(),
    "the feed sits under the layout declaring the slot"
  );
  assert!(
    host
      .intercept_for("/feed/photo/3", Some("/feed/photo/2"), None)
      .is_some(),
    "so does another photo"
  );
  assert!(
    host.intercept_for("/feed/photo/3", Some("/"), None).is_none(),
    "the index has no such layout"
  );
  assert!(
    host.intercept_for("/feed/photo/3", None, None).is_none(),
    "no origin is a document"
  );
  assert!(
    host.intercept_for("/feed/photo/3", None, Some("modal")).is_some(),
    "`into` names the slot outright"
  );
  assert!(
    host
      .intercept_for("/feed/photo/3", Some("/feed"), Some("panel"))
      .is_none(),
    "a slot the route has no variant for"
  );
  let (drawer, _) = host.intercept_for("/feed/photo/3", None, Some("drawer")).unwrap();
  assert_eq!(
    drawer.children[0].1.children[0].0.0, "drawer",
    "`into` picks among a route's variants"
  );
  let (first, _) = host.intercept_for("/feed/photo/3", Some("/feed"), None).unwrap();
  assert_eq!(
    first.children[0].1.children[0].0.0, "modal",
    "without `into`, the first variant whose layout the origin shares"
  );
  assert!(
    host.intercept_for("/feed", Some("/feed"), None).is_none(),
    "a route without a variant"
  );

  let (peek, _) = host
    .intercept_for("/deck/card/3", None, Some("peek"))
    .expect("a slot a nested layout declares");
  assert_eq!(peek.children[0].1.children[0].1.children[0].0.0, "peek");
  assert!(
    host.intercept_for("/deck/card/3", Some("/deck"), None).is_some(),
    "the origin shares both layouts down to the declaring one"
  );
  assert!(
    host.intercept_for("/deck/card/3", Some("/"), None).is_none(),
    "the index shares neither"
  );
  assert!(
    host.intercept_for("/deck/card/3", None, Some("content")).is_none(),
    "the layout on the way down is not the declaring one"
  );

  let payload = host
    .render_navigation_to_string("/feed/photo/3", Some("/feed"), None, SessionCell::default())
    .await
    .unwrap();
  let sidecar = payload.lines().find(|l| l.starts_with("G ")).unwrap();
  assert!(sidecar.contains("\"keep\":[\"content\",\"drawer\"]"), "{sidecar}");
  let plain = host
    .render_navigation_to_string("/feed/photo/3", Some("/"), None, SessionCell::default())
    .await
    .unwrap();
  ...
  let response = host
    .handle(
      Request::get("/feed/photo/3?__payload")
        .header("x-sf-from", "/feed?x=1")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  ...
  assert!(
    body.contains("\"keep\":[\"content\",\"drawer\"]"),
    "the edge reads the origin header: {body}"
  );
  let response = host
    .handle(Request::get("/feed/photo/3?__payload").body(Bytes::new()).unwrap())
    .await;
  let body = String::from_utf8(response.into_body().collect().await.unwrap().to_bytes().to_vec()).unwrap();
  assert!(!body.contains("keep"), "without it the payload is the page's: {body}");
  let response = host
    .handle(
      Request::get("/feed/photo/3")
        .header("x-sf-from", "/feed")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  let body = String::from_utf8(response.into_body().collect().await.unwrap().to_bytes().to_vec()).unwrap();
  assert!(
    body.contains("<!doctype html>") && !body.contains("page.modal"),
    "a document is never intercepted: {body}"
  );
}
...
#[tokio::test]
async fn a_source_reads_the_path_the_request_carried() {
  let (host, _) = host();
  let html = host
    .render_to_string("/where?view=active", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("\"/where\""),
    "the loader read the path, and the query is not part of it: {html}"
  );
}
...
#[tokio::test]
async fn the_path_a_source_reads_keeps_the_locale_prefix() {
  let host = localised();
  let html = host
    .render_to_string("/fr_FR/where", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("\"/fr_FR/where\""),
    "a link built from it stays in the locale the reader asked for: {html}"
  );

  let html = host
    .render_to_string("/where", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("\"/where\""),
    "and an unprefixed request carries no prefix: {html}"
  );

  let html = host
    .render_to_string("/en-us/where", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("\"/en_US/where\""),
    "a prefix is spelled the way the configuration spells it: {html}"
  );
}
...
async fn a_locale_prefix_is_stripped_and_marks_the_document_and_every_segment() {
  ...
  assert_eq!(
    host.report().locales,
    vec!["en_US".to_owned(), "fr_FR".to_owned(), "de".to_owned()]
  );
  assert!(
    host
      .report()
      .to_string()
      .contains("locales   en_US (default, unprefixed), fr_FR, de"),
    "{}",
    host.report()
  );

  let html = host
    .render_to_string("/fr_FR/hello/norm?from=test", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("<html lang=\"fr-FR\" data-sf-locale=\"fr_FR\">"),
    "{html}"
  );
  assert!(
    html.contains("hi norm via test"),
    "the route matched without its prefix: {html}"
  );
  assert!(html.contains("<!--sf-g:shell#document@fr_FR-->"), "{html}");
  assert!(
    html.contains("<!--sf-g:routes/hello/page.tsx#default?name=norm&from=test@fr_FR-->"),
    "{html}"
  );
  assert!(!html.contains("rel=\"canonical\""), "{html}");

  let html = host
    .render_to_string("/hello/norm?from=test", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("<html lang=\"en-US\" data-sf-locale=\"en_US\">"),
    "{html}"
  );
  assert!(
    html.contains("<!--sf-g:shell#document-->"),
    "the default locale leaves the key bare: {html}"
  );
  ...
  assert!(
    html.contains("<html lang=\"en-US\" data-sf-locale=\"en_US\">"),
    "the prefix matches whatever its spelling: {html}"
  );
  assert!(
    html.contains("<link rel=\"canonical\" href=\"/hello/norm\">"),
    "a prefixed default locale points at the bare path: {html}"
  );
  assert!(html.contains("hi norm via test"), "{html}");

  let payload = host
    .render_to_string("/de/hello/norm", RenderMode::Payload, SessionCell::default())
    .await
    .unwrap();
  ...
  assert!(
    matches!(
      host
        .render_to_string("/ja/hello/norm", RenderMode::Html, SessionCell::default())
        .await,
      Err(snapfire_fsr_host::HostError::NotFound(_))
    ),
    "an unsupported prefix is a path"
  );
}
...
async fn the_edge_takes_the_locale_from_the_prefix_the_cookie_then_the_header_and_remembers_a_prefix() {
  ...
  let response = host
    .handle(Request::get("/fr_FR/hello/x?from=y").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(response.status(), StatusCode::OK);
  assert_eq!(
    response.headers().get("x-locale").unwrap(),
    "fr_FR",
    "the middleware reads the locale"
  );
  assert_eq!(
    response.headers().get("x-path").unwrap(),
    "/hello/x",
    "and the stripped path"
  );
  ...
  assert!(
    cookies
      .iter()
      .any(|c| c.starts_with("sf_locale=fr_FR; Path=/; Max-Age=")),
    "the prefix is remembered: {cookies:?}"
  );
  ...
  let response = host
    .handle(
      Request::get("/hello/x?from=y")
        .header(header::COOKIE, "sf_locale=fr_FR")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  assert_eq!(
    response.headers().get("x-locale").unwrap(),
    "fr_FR",
    "an unprefixed path follows the cookie"
  );
  assert!(
    !response
      .headers()
      .get_all(header::SET_COOKIE)
      .iter()
      .any(|v| v.to_str().unwrap().starts_with("sf_locale=")),
    "nothing to remember"
  );
  ...
  let response = host
    .handle(
      Request::get("/hello/x?from=y")
        .header(header::ACCEPT_LANGUAGE, "ja, de-AT;q=0.8, fr;q=0.5")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  assert_eq!(
    response.headers().get("x-locale").unwrap(),
    "de",
    "the header's best supported language"
  );

  let response = host
    .handle(Request::get("/hello/x?from=y").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
    response.headers().get("x-locale").unwrap(),
    "en_US",
    "nothing says: the default"
  );

  let response = host
    .handle(
      Request::get("/fr_FR/hello/x?from=y")
        .header(header::COOKIE, "sf_locale=fr_FR")
        .body(Bytes::new())
        .unwrap(),
    )
  ...
  assert!(
    !response
      ...
      .iter()
      .any(|v| v.to_str().unwrap().starts_with("sf_locale=")),
    "the cookie already holds the prefix's locale"
  );

  let response = host
    .handle(Request::get("/fr_FR/static/app.js").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
    response.status(),
    StatusCode::NOT_FOUND,
    "static roots are never prefixed"
  );
  let response = host
    .handle(
      Request::post("/fr_FR/_sf/action/index.where")
        .body(Bytes::from("{}"))
        .unwrap(),
    )
    .await;
...
async fn an_action_runs_in_the_locale_of_the_document_that_called_it() {
  ...
  let response = host
    .handle(
      Request::post("/_sf/action/index.where")
        .header("x-sf-from", "/fr_FR/hello/x?from=y")
        .body(Bytes::from("{}"))
        .unwrap(),
    )
    .await;
  ...
  let response = host
    .handle(
      Request::post("/_sf/action/index.where")
        .header("x-sf-from", "/hello/x")
        .header(header::COOKIE, "sf_locale=de")
        .body(Bytes::from("{}"))
        .unwrap(),
    )
    .await;
  ...
  assert!(
    !response
      ...
      .iter()
      .any(|v| v.to_str().unwrap().starts_with("sf_locale=")),
    "an action never writes the locale cookie"
  );
...
async fn prerender_writes_every_locale_and_the_edge_serves_each_from_its_own_directory() {
  ...
  for path in ["/", "/fr_FR", "/de"] {
    assert_eq!(
      served.iter().filter(|p| **p == path).count(),
      2,
      "a document and a payload for {path}: {served:?}"
    );
  }
  ...
  assert!(
    host
      .prerendered("/", RenderMode::Html)
      .unwrap()
      .contains("lang=\"en-US\"")
  );
  assert!(
    host
      .prerendered("/fr_FR", RenderMode::Html)
      .unwrap()
      .contains("lang=\"fr-FR\"")
  );
  assert!(
    host
      .prerendered("/fr-fr/", RenderMode::Html)
      .unwrap()
      .contains("lang=\"fr-FR\"")
  );
  assert!(
    host
      .prerendered("/de", RenderMode::Payload)
      .unwrap()
      .contains("\nL \"de\"\n")
  );
  ...
  assert_eq!(response.headers().get("x-sf-prerendered").unwrap(), "1");
  assert!(
    body_of(response).await.contains("lang=\"de\""),
    "the cookie picks the prerendered locale"
  );
...
fn a_bad_locales_section_refuses_to_start() {
  ...
  assert!(
    err.contains("locales.default `fr` is not among locales.supported"),
    "{err}"
  );
}
...
fn cookie_of(response: &http::Response<snapfire_fsr_host::Body>) -> String {
  let set = response
    ...
    .get_all(header::SET_COOKIE)
    .iter()
    .find(|v| v.to_str().unwrap().starts_with("sf_session="))
...
async fn the_login_flow_signs_a_session_in_through_the_file_provider() {
  ...
  assert_eq!(
    host.report().bearer,
    vec![("shop".to_owned(), "access_token".to_owned())]
  );
  let report = host.report().to_string();
  assert!(report.contains("auth      file, login page /login"), "{report}");
  assert!(
    report.contains("bearer    shop                   access_token"),
    "{report}"
  );
  ...
  assert_eq!(
    transport.last_metadata("authorization"),
    None,
    "an anonymous call carries no bearer"
  );

  let response = host
    .handle(Request::get("/auth/login?return_to=/who").body(Bytes::new()).unwrap())
    .await;
  ...
  let response = host
    .handle(
      Request::get("/login?return_to=%2Fwho")
  ...
  assert_eq!(
    response.status(),
    StatusCode::OK,
    "the login page is the application's route"
  );
  ...
  assert_eq!(
    transport.last_metadata("authorization").as_deref(),
    Some("Bearer dev-token-alice"),
    "the loader's call carried the bearer"
  );
  ...
  assert!(
    expiring.starts_with("sf_session=;") && expiring.contains("Max-Age=0"),
    "{expiring}"
  );
...
async fn a_wrong_password_returns_to_the_login_page_and_a_replay_is_invalid() {
  ...
  let response = host
    .handle(
      Request::get("/login")
        .header(header::COOKIE, &cookie)
        .header(header::REFERER, "http://localhost/who?x=1")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  ...
  assert_eq!(
    location(&response),
    "/who?x=1",
    "the login page reseeded the flow from the referer"
  );

  let response = host
    .handle(
      Request::get("/auth/callback?user=bob&password=builder")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  assert_eq!(
    response.status(),
    StatusCode::BAD_REQUEST,
    "a callback with no flow in progress"
  );
}
...
async fn return_to_never_leaves_the_origin_and_the_routes_are_never_prefixed() {
  ...
  for bad in ["https://evil.example/x", "//evil.example", "evil"] {
    let response = host
      .handle(
        Request::get(format!("/auth/login?return_to={bad}"))
          .body(Bytes::new())
          .unwrap(),
      )
      .await;
  ...
  let response = host
    .handle(
      Request::get("/auth/login")
        .header(header::REFERER, "http://localhost/who")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  ...
  let response = host
    .handle(Request::get("/fr_FR/auth/login").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(response.status(), StatusCode::NOT_FOUND);

  let (plain, _) = self::host();
  let response = plain
    .handle(Request::get("/auth/login").body(Bytes::new()).unwrap())
    .await;
...
fn the_auth_section_and_the_users_file_are_checked() {
  ...
  let err = Host::from(dir.join("app.toml"))
    ...
    .build()
    .err()
    .expect("an empty table is an error")
    .to_string();
  ...
  std::fs::write(
    ...
    std::fs::read_to_string(dir.join("app.toml"))
      .unwrap()
      .replace("provider = \"file\"", "provider = \"oidc\""),
  )
  .unwrap();
  let err = Host::from(dir.join("app.toml"))
    .err()
    .expect("an unknown provider is an error")
    .to_string();
...
impl snapfire_fsr_runtime::Metadata for ItemsTitle {
  fn describe(
    ...
    data: &snapfire_fsr_core::Data,
  ) -> futures::future::BoxFuture<'static, Result<snapfire_fsr_runtime::Meta, snapfire_fsr_runtime::LoadError>> {
    ...
    Box::pin(async move {
      Ok(snapfire_fsr_runtime::Meta {
        title: Some(format!("{count} items for {who}")),
        description: None,
        head: Vec::new(),
      })
    })
...
fn formed() -> Arc<Host> {
  ...
  let host = Host::from(formed_dir().join("app.toml"))
    ...
    .action("recall", |ctx, _input| async move {
      Ok(ctx.session.get("word").unwrap_or(Value::Null))
    })
...
async fn a_form_posts_an_action_with_its_token_and_lands_back_on_the_referer() {
  ...
  assert!(
    !cookie.is_empty(),
    "and establishes the session so the token verifies next time"
  );
  assert!(
    html.contains("<title>2 items for nobody</title>"),
    "the Rust metadata titled the document: {html}"
  );
  ...
  let response = host
    .handle(
      Request::post("/_sf/action/recall")
        .header(header::COOKIE, &cookie)
        .header(header::CONTENT_TYPE, "application/json")
  ...
  assert_eq!(
    ...
    "\"hi\"",
    "the form's field reached the action as a string and the session kept it"
  );
...
async fn a_payload_request_may_only_name_an_encoding_that_exists() {
  let host = formed();
  let response = host
    .handle(Request::get("/?__payload&enc=cbor").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(response.status(), StatusCode::NOT_ACCEPTABLE);
  assert!(body_of(response).await.contains("unsupported payload encoding `cbor`"));
  let response = host
    .handle(Request::get("/?__payload&enc=json").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(response.status(), StatusCode::OK);
  assert!(body_of(response).await.starts_with("V {\"fmt\":"));
  let response = host
    .handle(Request::get("/?enc=cbor").body(Bytes::new()).unwrap())
    .await;
  ...
  assert!(
    !body_of(response).await.contains("csrf_token"),
    "csrf = identified is the default"
  );
}
...
async fn a_mock_client_answers_from_its_responses_file_and_reaches_nothing() {
  let host = Host::from(mocked_dir(r#"{"list": ["socks", "hat"]}"#, "mock").join("app.toml"))
    .unwrap()
    .build()
    .unwrap();
  assert_eq!(
    host.report().services,
    vec![(
      "shop".to_owned(),
      "mock".to_owned(),
      "clients/shop.mock.json".to_owned()
    )]
  );
  let report = host.report().to_string();
  assert!(
    report.contains("services  shop                   mock        clients/shop.mock.json"),
    "{report}"
  );
  ...
  let html = body_of(response).await;
  assert!(
    html.contains("socks") && html.contains("hat"),
    "the loader's call answered from the file: {html}"
  );
}
...
async fn a_mock_response_can_be_a_failure_and_the_transport_name_is_checked() {
  let host = Host::from(
    mocked_dir(
      r#"{"list": {"$fail": {"kind": "unavailable", "message": "the shop is closed"}}}"#,
      "mock",
    )
    .join("app.toml"),
  )
  ...
  let html = body_of(response).await;
  assert!(
    html.contains("the shop is closed"),
    "the failure reached the render: {html}"
  );

  let error = Host::from(mocked_dir("{}", "smtp").join("app.toml"))
    .err()
    .expect("an unknown transport is refused")
    .to_string();
  assert!(
    error.contains("clients.shop.transport") && error.contains("smtp"),
    "{error}"
  );
}
...
impl snapfire_fsr_service::Transport for IdentityService {
  ...
  ) -> futures::future::BoxFuture<'static, Result<Value, snapfire_fsr_runtime::ServiceError>> {
    ...
    let result = match path.as_str() {
      ...
      "identity.putSession" => {
        self
          .sessions
          .lock()
          .insert(str_arg(&call.args, "id"), str_arg(&call.args, "record"));
        Ok(Value::Null)
      }
...
async fn sessions_and_sign_in_live_behind_the_identity_client() {
  ...
  let host = Host::from(remote_dir().join("app.toml"))
    .unwrap()
    .services_over(identity.clone())
    .build()
    .unwrap();
  let report = host.report().to_string();
  assert!(report.contains("session   service via identity"), "{report}");
  assert!(
    report.contains("auth      service via identity, login page /login"),
    "{report}"
  );
  ...
  let cookie = cookie_of(&response);
  assert_eq!(
    identity.sessions.lock().len(),
    1,
    "the flow state was stored through putSession"
  );
  ...
  let response = host
    .handle(
      Request::post("/auth/callback")
        .header(header::COOKIE, &cookie)
        .header(header::CONTENT_TYPE, "application/x-www-form-urlencoded")
        .body(Bytes::from("user=alice&password=nope"))
        .unwrap(),
    )
    .await;
  assert_eq!(
    location(&response),
    "/login?error=denied&return_to=%2Fwho",
    "the service's 401 is a denial"
  );
  ...
  let stored = identity
    ...
    .next()
    .cloned()
    .expect("the session is in the service");
  assert!(
    stored.contains("\"alice\"") && stored.contains("svc-token-alice"),
    "identity and custody travel in the record: {stored}"
  );
  ...
  assert!(
    html.contains("alice"),
    "the loader read the identity the service holds: {html}"
  );
  assert_eq!(
    identity.shop_bearer.lock().as_deref(),
    Some("Bearer svc-token-alice"),
    "the token the service issued rides the shop call"
  );
  ...
  assert_eq!(
    identity.sessions.lock().len(),
    0,
    "logout deleted the session in the service"
  );
}
...
fn a_service_store_or_provider_needs_a_client_that_exists() {
  let dir = remote_dir();
  let text = std::fs::read_to_string(dir.join("app.toml"))
    .unwrap()
    .replace("client = \"identity\"\n\n[auth]", "client = \"vault\"\n\n[auth]");
  std::fs::write(dir.join("app.toml"), text).unwrap();
  let error = Host::from(dir.join("app.toml"))
    .err()
    .expect("a client that is not declared is refused")
    .to_string();
...
async fn a_cached_method_answers_renders_from_memory_until_a_write_or_a_drop() {
  ...
  let report = host.report().to_string();
  assert!(
    report.contains("cached    shop.list              ttl 1m shared [items]"),
    "{report}"
  );
...
async fn a_reload_swaps_the_tables_in_place_and_keeps_the_sessions() {
  ...
  let cookie = response
    ...
    .to_owned();
  let html = host
    .render_to_string("/hello/x?from=y", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("hi x via y") && html.contains("<title>Test &lt;app&gt;</title>"),
    "{html}"
  );
  ...
  let response = host
    .handle(Request::post("/__fsr/reload").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(response.status(), StatusCode::OK);
  let printed = body_of(response).await;
  assert!(
    printed.contains("lowered"),
    "the reload answers with the new report: {printed}"
  );
  ...
  let html = host
    ...
    .unwrap();
  assert!(
    html.contains("hey x via y") && html.contains("<title>Reloaded</title>"),
    "the new plan and head serve: {html}"
  );
  let response = host
    .handle(
      Request::post("/_sf/action/index.bump")
  ...
  let html = host
    ...
    .unwrap();
  assert!(
    html.contains("hey x via y"),
    "a refused reload leaves the tables alone: {html}"
  );
...
fn a_bundle_carrying_a_server_module_refuses_to_start() {
  ...
  let leaked =
    build(r#"{"outputs": ["routes/index/page.js", "routes/index/page.loader.js"], "graph": {}}"#).unwrap_err();
  assert!(
    leaked
      .contains("server modules in the bundle: routes/index/page.loader.js is a loader, routes/index/page.loader.ts"),
    "{leaked}"
  );
  let imported =
    build(r#"{"outputs": ["routes/index/page.js"], "graph": {"routes/index/page.js": ["routes/index/actions.js"]}}"#)
      .unwrap_err();
  assert!(
    imported.contains("routes/index/page.js imports routes/index/actions.js"),
    "{imported}"
  );
...
async fn a_site_serves_standalone_under_its_prefix_with_its_ids_prefixed() {
  ...
  let html = host
    .render_to_string("/shop", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(
    html.contains("data-sf-module=\"shop:routes/index/page.tsx#default\"") && html.contains("\"a\""),
    "{html}"
  );
  ...
  let html = host
    .render_to_string("/shop/hello/x?from=y", RenderMode::Html, SessionCell::default())
    .await
    .unwrap();
  assert!(html.contains("hi x via y"), "{html}");
  assert!(
    host
      .render_to_string("/hello/x", RenderMode::Html, SessionCell::default())
      .await
      .is_err(),
    "nothing serves outside the prefix"
  );

  let response = host
    .handle(
      Request::post("/_sf/action/shop:index.bump")
        .header(header::CONTENT_TYPE, "application/json")
        .body(Bytes::from(r#"{"by": 2}"#))
        .unwrap(),
    )
...
async fn a_mounted_site_serves_under_the_shells_root_layout_with_its_own_middleware_head_and_clients() {
  ...
  std::fs::write(
    ...
    toml.replace(
      "entry = \"/static/app.js\"",
      "entry = \"/shop/static/js/app/src/main.js\"",
    ),
  )
  .unwrap();
  ...
  let report = host.report().to_string();
  assert!(
    report.contains("sites     shop                   at /shop from") && report.contains("dev deadbeef"),
    "{report}"
  );
  assert!(report.contains("ignored [static /static, session]"), "{report}");
  assert!(
    report.contains("/shop/hello/{name}")
      && report.contains("shop:index")
      && report.contains("static    /static")
      && report.contains("/shop/static/css"),
    "{report}"
  );
  ...
  assert_eq!(response.status(), StatusCode::OK);
  assert_eq!(
    response.headers().get("x-shell").unwrap(),
    "shop",
    "the shell's middleware saw the site"
  );
  assert_eq!(
    response.headers().get("x-site").unwrap(),
    "shop",
    "the site's middleware ran after it"
  );
  let html = body_of(response).await;
  assert!(
    html.contains("<header>the shell</header>"),
    "the shell's root layout wraps the site: {html}"
  );
  assert!(
    html.contains("data-sf-module=\"shop:routes/index/page.tsx#default\"") && html.contains("\"b\""),
    "the site's page rendered inside it through its own client: {html}"
  );
  assert!(
    html.contains("<link rel=\"stylesheet\" data-sf-css=\"\" href=\"/shop/static/css/site.css\">")
      && html.contains("<script type=\"module\" src=\"/shop/static/js/app/src/main.js\"></script>"),
    "{html}"
  );
  let (before, after) = html.split_once("<title>").expect("a title");
  assert!(
    !before.contains("/shop/static/css/site.css") && after.contains("/shop/static/css/site.css"),
    "the site's stylesheet comes after the document's own, so its rules win: {html}"
  );

  let response = host.handle(Request::get("/").body(Bytes::new()).unwrap()).await;
  assert_eq!(response.headers().get("x-shell").unwrap(), "none");
  assert!(
    response.headers().get("x-site").is_none(),
    "the site's middleware stays under its prefix"
  );
  let html = body_of(response).await;
  assert!(
    !html.contains("/shop/static/js/app/src/main.js") && html.contains("\"a\""),
    "the shell's page renders through the shell's client: {html}"
  );

  let payload = body_of(
    host
      .handle(
        Request::get("/shop/hello/x?from=y&__payload")
          .body(Bytes::new())
          .unwrap(),
      )
      .await,
  ...
  .await;
  assert!(
    payload.contains("E \"/shop/static/js/app/src/main.js\"") && payload.contains("hi x via y"),
    "{payload}"
  );
  let payload = body_of(
    host
  ...
  let response = host
    .handle(Request::get("/shop/static/css/site.css").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
    response.status(),
    StatusCode::OK,
    "the site's stylesheet is served under its prefix"
  );
  let response = host
    .handle(
      Request::post("/_sf/action/shop:index.bump")
  ...
  assert_eq!(
    body_of(response).await,
    "4",
    "the site's action runs under its prefixed id"
  );
  let status = body_of(
    host
      .handle(Request::get("/__fsr/sites").body(Bytes::new()).unwrap())
      .await,
  )
  .await;
  assert!(
    status.contains("\"name\":\"shop\"") && status.contains("\"hash\":\"deadbeef\""),
    "{status}"
  );
}
...
async fn a_registered_native_pair_answers_a_lowered_body_and_an_unregistered_one_refuses_to_build() {
  ...
  let err = Host::from(dir.join("app.toml"))
    ...
    .unwrap_or_default();
  assert!(
    err.contains("`index` calls extension `fmt.pretty`, which nothing registers"),
    "{err}"
  );
  ...
  let host = Host::from(dir.join("app.toml"))
    ...
    .unwrap();
  let payload = host
    .render_to_string("/", RenderMode::Payload, SessionCell::default())
    .await
    .unwrap();
  assert!(
    payload.contains("en:1"),
    "the pair ran under the default locale: {payload}"
  );
  assert!(
    payload.contains("1,234.5"),
    "the standard library is there too: {payload}"
  );
  ...
  assert!(
    host
      .report()
      .to_string()
      .contains("natives   fmt.pretty             rust"),
    "{}",
    host.report()
  );
}
...
async fn catalogs_under_locales_reach_the_document_the_payload_and_t() {
  ...
  assert!(
    host.report().to_string().contains("catalogs  en 4 keys"),
    "{}",
    host.report()
  );
  ...
  assert!(
    html.contains("Hello Norm") && html.contains("2 items"),
    "t ran in the loader: {html}"
  );
  ...
  assert!(
    payload.contains("\nD {\"hello.num\""),
    "a payload carries the catalog when nothing says it is held: {payload}"
  );

  let host = Arc::new(host);
  let held = host
    .handle(
      Request::get("/?__payload")
        .header("x-sf-catalog", "en")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  ...
  assert!(
    text.contains("\nL \"en\"") && !text.contains("\nD "),
    "the row is dropped for a navigator holding it: {text}"
  );
  let missing = host
    .handle(
      Request::get("/?__payload")
        .header("x-sf-catalog", "fr")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  assert!(
    body_of(missing).await.contains("\nD {"),
    "a navigator holding another locale gets it"
  );
}
...
async fn a_route_reading_only_the_identity_prerenders_for_anonymous_visitors_and_renders_live_for_a_signed_in_one() {
  ...
  assert_eq!(
    host.prerenderable_anonymous(),
    vec!["/who".to_owned()],
    "the loader reads identity and calls a bearer client, nothing else"
  );
  assert!(
    host.report().to_string().contains("/who                   ")
      && host.report().to_string().contains("for anonymous visitors"),
    "{}",
    host.report()
  );
  ...
  let response = host.handle(Request::get("/who").body(Bytes::new()).unwrap()).await;
  assert_eq!(
    response.headers().get("x-sf-prerendered").map(|v| v.to_str().unwrap()),
    Some("1")
  );
  ...
  let response = host
    ...
    .await;
  assert!(
    response.headers().get("x-sf-prerendered").is_none(),
    "a signed-in visitor is rendered live"
  );
  let html = body_of(response).await;
  ...
  assert_eq!(
    response.headers().get("x-sf-prerendered").map(|v| v.to_str().unwrap()),
    Some("1"),
    "a route reading nothing serves its file to everyone"
  );
...
mod tls {
  ...
  async fn a_reload_swaps_the_certificate_and_a_bad_one_leaves_the_running_certificate() {
    ...
    assert_eq!(
      presented,
      first_der("b"),
      "the next handshake presents what the files now hold"
    );
    assert!(body.contains("hi tls via two"), "{body}");
    assert!(
      get(addr, "a", "/hello/tls?from=old").await.is_err(),
      "a client trusting only the old certificate is refused"
    );
    ...
    assert_eq!(
      presented,
      first_der("b"),
      "a failed reload leaves the running certificate in place"
    );
  }
  ...
  fn first_der(pair: &str) -> Vec<u8> {
    use rustls_pki_types::pem::PemObject;
    rustls_pki_types::CertificateDer::from_pem_slice(&fixture(&format!("{pair}-cert.pem")))
      .unwrap()
      .to_vec()
  }
}
...
async fn live_streams_the_topics_a_listener_asked_for_and_nothing_else() {
  ...
  let response = host
    .handle(
      Request::get("/_sf/live?topics=board,weather")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
...
async fn live_needs_topics_and_publishing_to_nobody_costs_nothing() {
  ...
  let body = response.into_body().collect().await.unwrap().to_bytes();
  assert!(
    String::from_utf8_lossy(&body).contains("topics=a,b"),
    "the error says how to ask"
  );
}
...
async fn a_topic_rule_decides_who_may_follow_what() {
  ...
  let response = host
    .handle(Request::get("/_sf/live?topics=room/42").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
    response.status(),
    StatusCode::FORBIDDEN,
    "a visitor in no room follows no room"
  );
  let body = response.into_body().collect().await.unwrap().to_bytes();
  assert!(
    String::from_utf8_lossy(&body).contains("room/42"),
    "the refusal names the topic"
  );

  let response = host
    .handle(Request::get("/_sf/live?topics=board").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
    response.status(),
    StatusCode::OK,
    "a topic the rule allows is still open to anyone"
  );

  let response = host
    .handle(
      Request::get("/_sf/live?topics=board,room/42")
        .body(Bytes::new())
        .unwrap(),
    )
    .await;
  assert_eq!(
    response.status(),
    StatusCode::FORBIDDEN,
    "one refused topic refuses the stream rather than half of it"
  );
}
...
async fn a_socket_is_refused_without_a_handler_a_topic_or_the_rule_s_blessing() {
  ...
  let (host, _) = host();
  let response = host
    .handle(Request::get("/_sf/socket?topic=wave/1").body(Bytes::new()).unwrap())
    .await;
  ...
  let response = host
    .handle(Request::get("/_sf/socket").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
    response.status(),
    StatusCode::BAD_REQUEST,
    "a socket is opened on one topic"
  );

  let response = host
    .handle(Request::get("/_sf/socket?topic=wave/2").body(Bytes::new()).unwrap())
    .await;
  assert_eq!(
    response.status(),
    StatusCode::FORBIDDEN,
    "the same rule guards the socket and the stream"
  );
  ...
  assert_eq!(
    response.status(),
    StatusCode::BAD_REQUEST,
    "an allowed topic still needs a handshake"
  );
  assert_eq!(host.sockets().on("wave/1"), 0, "nothing was opened");
}

#[tokio::test]
async fn a_payload_into_a_site_carries_its_stylesheets_and_one_out_of_it_carries_none() {
  let site = site_dir();
  let host = shell_with(&site);

  let payload = host
    .render_to_string("/shop", RenderMode::Payload, SessionCell::default())
    .await
    .unwrap();
  let row = payload
    .lines()
    .find(|line| line.starts_with("C "))
    .expect(&format!("a C row: {payload}"));
  assert_eq!(row, "C [\"/shop/static/css/site.css\"]");
  assert!(
    payload.lines().any(|line| line.starts_with("E ")),
    "the entry still rides its own row: {payload}"
  );

  // A route outside the site names no stylesheet at all, which is what tells
  // the browser to drop the ones the site added.
  let payload = host
    .render_to_string("/", RenderMode::Payload, SessionCell::default())
    .await
    .unwrap();
  assert!(!payload.lines().any(|line| line.starts_with("C ")), "{payload}");
}

#[tokio::test]
async fn an_application_with_no_document_section_still_renders_a_document() {
  // `document.shell` carries a `serde(default)` naming a function, which a
  // fallback to `Default` drops: the empty module then matches no evaluator
  // and the document shell never runs, so the response is a bare fragment.
  let dir = app_dir();
  let toml = std::fs::read_to_string(dir.join("app.toml")).unwrap();
  let stripped: String = toml
    .lines()
    .filter(|line| !line.starts_with("[document]") && !line.starts_with("title =") && !line.starts_with("entry ="))
    .collect::<Vec<_>>()
    .join("\n");
  std::fs::write(dir.join("app.toml"), stripped).unwrap();

  let config = snapfire_fsr_host::config::Config::load(&dir.join("app.toml")).unwrap();
  assert_eq!(config.document.shell, "shell#document");
