...
`vue::VueLowerer` reads a `.vue` file the plugin described and builds the same `Component` a `.tsx` module builds, with `hydrated_by` `Some(HydratedBy::Vue)` and no hoisting. `ComponentSet::lower` takes this path for a described `.vue` module; the export must be `default`.

* `vue::VUE_CLIENT: &str`, `@snapfire/fsr-client/vue`, where `useStore` is imported from.
* `vue::Template { children: Vec<Node> }`, `vue::Node` and `vue::Prop`: the description's template tree as `snapfirec-vue`'s `describeNode` writes it, read from `Described::template` with serde. A `Node` is `element` with `tag`, `kind` (Vue's `ElementTypes`: element 0, component 1, slot 2, template 3), `props` and `children`, `text` with `content`, `interpolation` with `content` or `other` with Vue's node `type`; a `Prop` is `attribute` with `name` and `value` or `directive` with `name`, `arg`, `argStatic`, `exp` and `modifiers`. Every node and prop carries its `line` and `column` in the file; a directive carries `expLine` and `expColumn` for its expression as well.
* The script block is parsed as TypeScript padded to its line in the file; each template expression is parsed with `Parsed::parse_expr_at` padded to its line and column, so a residue names the `.vue` file's own position.
* `<script setup>`: `defineProps`, bound whole or destructured, is `$props`, with `withDefaults` adding a `let $props` that spreads the defaults under the placement's props; `ref` and `shallowRef` are a `let` and a state binding, read as the value in the template and through `.value` in the script; `computed` of an arrow is a `let`; `reactive`, `shallowReactive` and `readonly` bind their value; `useStore(key, initial)` from `VUE_CLIENT` is `let name = <store key> ?? initial`, a state binding read through `.value` on both sides; a `const` with any other value is a `let`; a function, `defineEmits` and the lifecycle, watch, `provide`, `nextTick`, `defineExpose`, `defineOptions` and `defineSlots` calls are the browser's and are passed over; an interface or a type alias is nothing. Residue: an export, a class, an enum, an array destructuring, `defineModel`, `toRef`, `toRefs`, `customRef`, `useTemplateRef`, `useId`, `inject`, `useAttrs`, `useSlots`, a writable `computed`, a plain `<script>` beside or instead of `<script setup>` and any other statement.
* The template: the root is the one child or a `Fragment`; text and interpolation as they stand; `v-if`, `v-else-if` and `v-else` chain into `If`, whitespace between branches dropped; `v-for="(item, index) in items"` is `For` with those params, the condition of a `v-if` on the same element lowered outside the loop; a `<template>` with `v-if` or `v-for` is its one element or a `Fragment`; a `<slot />` with no name and no bindings is `Slot("content")`, its fallback dropped since the island's children region is always written; a static attribute is a literal, valueless as `true`; a static `class` beside `:class` merges into it as `[dynamic, static]` at the dynamic one's position; a static `style` is parsed into an object; `:style`, `v-show` and a static `style` merge into one `style` array at the first one's position; `:class` becomes a one-element array; a bound boolean attribute (Vue's `isBooleanAttr` set) is `true` when truthy or the empty string and null otherwise; any other bound attribute is null when null and `String(value)` otherwise, under the name as written except the four camelCase spellings Vue maps; `:key`, `ref`, `v-on`, `v-once`, `v-memo` and `v-cloak` are dropped; `v-html` is `RAW_ATTR`; `v-text` replaces the children; a scoped style's `data-v-` id is a `true` attribute on every element. Residue: a dynamic attribute name, `v-bind` of a whole object, `.prop`, `:is`, `value` on a `<textarea>`, `v-model`, `v-slot`, `v-pre`, a custom directive, a named or bound `<slot>`, a component tag, a `<template>` with anything but those directives and a `key`, `v-else` with no `v-if` before it, a `v-for` alias that is a pattern or a third alias and a node type the plugin passed through unread.
