...
* A scaffold with no `--with` ends its `next` lines with `fsr use <app> react`, marked as wanted only by an application that wants React, with the other directions named, so nobody expecting the React project earlier versions wrote is left wondering where it went.
...
* `components` rows are module, owner and detail: owner `lowered`, `client` or `foreign`; for `client` and `foreign`, the detail is the residue's `file:line:column`; for `lowered`, the detail is `static` when the template has no state, no handlers and no component inline that has them, so nothing mounts it, `tree` for a layout declared `tree(Layout)`, which the React adapter mounts as one root with its page, `vue` for a `.vue` component the Vue front end lowered; otherwise it is empty.
...
* `generated/fsr.ts` is what the generated `tsconfig.json` maps `@snapfire/fsr` to; it imports the base package as `@snapfire/fsr-authoring`, re-exports `fail` and `Services`, imports `Session`, declares `Routes` with one key per pattern whose value has a `string` field per parameter, `Ctx<P extends keyof Routes = keyof Routes>` with `params`, `query`, `session`, `identity`, `locale`, `services` and `now`, `ActionCtx<Input, P>` and an `action<Input, Out>` wrapper over `@snapfire/fsr`'s. `ActionCtx`, `HandlerCtx<P>` and `MiddlewareCtx` type `session` as `Session & SessionControl`, which adds `extend(seconds: number)`; `Ctx` on its own does not, so a loader cannot call it and the lowerer refuses one that tries. A `route.ts` handler that extends types its parameter `HandlerCtx<"/api/cart">` in place of `Ctx<"/api/cart">`.
...
* `VueMajor { version: String, supported: String }`, a vendored Vue whose markup the renderer does not write.
* `Plugin(String)`, a framework plugin that started and then failed to answer a describe batch, with the host's message.
