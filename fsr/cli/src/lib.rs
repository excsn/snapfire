...
pub fn build(app: &Path, options: &Options) -> Result<Built, BuildError> {
  ...
  files.extend([
    ...
    ("generated/islands.ts".to_owned(), islands_module(&islands, &static_modules, options)),
  ...
  if let Some(shim) = foreign_declarations(&foreign) {
    files.push(("generated/foreign.d.ts".to_owned(), shim));
  }
...
fn islands_module(islands: &[String], static_modules: &[String], options: &Options) -> String {
  let mut out = String::from("// Generated by fsr build. Do not edit.\n\n");
  ...
  for module in registered {
    ...
    // `generated/foreign.d.ts` and snapfirec rewrites the specifier to the
...
/// `generated/foreign.d.ts`: what an import of a component the build does not
/// read is, for the typechecker. A `.vue` file is a Vue component; the plugin
/// that compiles it gives it no declaration of its own.
fn foreign_declarations(foreign: &[String]) -> Option<String> {
  let mut extensions: Vec<&str> = foreign.iter().filter_map(|m| Path::new(m.split_once('#').map(|(f, _)| f).unwrap_or(m)).extension().and_then(|e| e.to_str())).collect();
  extensions.sort();
  extensions.dedup();
