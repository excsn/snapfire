...
Not every route module is in it. A template the report marks `static`, one with no state and no handlers and nothing inline that has them, has no browser twin, so the build lists the route files that do hydrate in `tsconfig.build.json` rather than `routes/**/*.tsx`. An application whose templates are all static bundles `src/` and the two generated files and nothing else. A `page.tera` or `layout.tera` is further out still: the report lists it as `template`, nothing is lowered, bundled or typechecked for it and the host renders it from the file, while the island modules it places by literal are bundled like any other.
...
A route whose loader reads no parameter, no query, no session, no identity and no clock renders the same for every request. The host can tell, because the loader is data it can read. The boot report lists such routes under `prerender`; `fsr prerender app` (or the project's binary with `--prerender`) renders each once into `server.prerender` and the host answers those from the file from then on, with `x-sf-prerendered: 1` so you can see it. A relative `server.prerender` is resolved against the app directory, so `dist/prerender` in a project-root config lands under `app/`. The storefront's `/about` qualifies; every other page reads the cart from the session, so none of them does. There is nothing to declare: a loader that starts reading the session takes its route back to per-request rendering at the next build. A route with a parameter is the one exception, since nothing can tell which values it takes: its page loader exports `paths` naming them and the command writes one file per set, `/blog/hello` and `/blog/world` for `/blog/{slug}`. Each run lists what it wrote in `prerendered.json` and removes those files before writing again, so a set that dropped out leaves no stale page. A route the host cannot write whole still gets what it can: under a layout reading the session, a page reading nothing is rendered once by the same command into `renders.json` and spliced in at request time, so only the layout is evaluated per request.
