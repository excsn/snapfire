...
At boot the host reads each artifact, the directory a site's build leaves behind, checks that it is the site it claims to be, refuses one carrying engine-owned rows or a leaked server module, nests its routes under the shell's root layout and adds its rows to the shell's tables. One document, one session, one navigation: a click from the portal's directory into `/billing` is a payload navigation that keeps the header's island and imports the site's islands on the way, from an `E` row the payload carries. The site's entry calls `enableNavigation` too, as every entry does and a second call on a document the navigator is already wired to leaves the spine of the navigation that loaded it in place, so the click after it patches as well.
