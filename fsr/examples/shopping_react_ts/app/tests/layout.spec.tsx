...
test("the layout hydrates over the page, keeps its state across a navigation and takes new props after an action", async () => {
  ...
  assert.equal(document.querySelector('sf-i[data-sf-module="routes/page.tsx#default"]'), null, "the page has no state, so it is markup rather than an island");
  assert.ok(layout!.querySelector("sf-s:not([data-sf-name]):not([data-sf-island])")?.textContent?.includes("Today's picks"), "the page sits in the layout's slot");
