...
test("a click on a product from the catalog opens it in the layout's modal slot and keeps the catalog", async () => {
  ...
  await load("/", { ctx: c });
  const catalog = document.querySelector('sf-i[data-sf-module="routes/index/page.tsx#default"]');
  ...
  assert.ok(document.querySelector('sf-i[data-sf-module="routes/index/page.tsx#default"]') === catalog, "the catalog kept its DOM");
