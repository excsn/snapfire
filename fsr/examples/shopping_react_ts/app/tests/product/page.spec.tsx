...
test("the product page hydrates with its quantity select", async () => {
  const r = await render(<ProductPage product={product} stock={stock} inCart={0n} cartCount={0n} />);
...
test("an action that fails is reported to the page, not the test", async () => {
  ...
  await render(<ProductPage product={sold} stock={{ ...stock, on_hand: 0n }} inCart={0n} cartCount={0n} />, { ctx: c });
