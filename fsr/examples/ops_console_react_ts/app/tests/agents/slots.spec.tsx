...
test("a full link renders the agent under the list, peek renders it beside the list, and the list stays", async () => {
  ...
  assert.equal(location.pathname, "/agents/1");
  assert.ok(document.querySelector('sf-i[data-sf-module="routes/agents/[id]/page.tsx#default"][data-sf-mounted]'), "the page took the content slot under the list");
  ...
  assert.equal(location.pathname, "/agents/3");
  assert.ok(peek.querySelector('sf-i[data-sf-module="routes/agents/[id]/page.peek.tsx#default"][data-sf-mounted]'), "`into` picked the variant the nested layout declares");
  ...
  assert.ok(document.querySelector('sf-i[data-sf-module="routes/agents/[id]/page.tsx#default"]'), "the page under the list stayed too");
  await settle();
});
...
test("a plain link from an alert peeks from the list and navigates from the summary", async () => {
  ...
  assert.equal(location.pathname, "/agents/3");
  assert.ok(peek.querySelector('sf-i[data-sf-module="routes/agents/[id]/page.peek.tsx#default"]'), "the origin shares the agents layout, so the server chose its slot");

  await load("/", { ctx: c });
  await settle();
  await fireEvent.click(screen.getByText("open"));
  assert.equal(location.pathname, "/agents/3");
  assert.ok(document.querySelector('sf-i[data-sf-module="routes/agents/[id]/page.tsx#default"][data-sf-mounted]'), "no agents layout on the summary to intercept into, so the page rendered whole");
  ...
  assert.equal(document.querySelector('sf-i[data-sf-module="routes/agents/[id]/page.peek.tsx#default"]'), null);
  await settle();
});
...
test("a region in the query survives both kinds of intercept", async () => {
  ...
  assert.equal(location.pathname + location.search, "/agents/1?region=eu");
...
test("a document load of an agent streams the page and the alerts behind their fallbacks", async () => {
  ...
  assert.ok(document.querySelector('sf-i[data-sf-module="routes/agents/[id]/page.tsx#default"][data-sf-mounted]'));
