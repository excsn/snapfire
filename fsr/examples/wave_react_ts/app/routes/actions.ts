...
/** A move on a board gadget. Everything a move is happens here and in the
...
export const play = action(async ({ input, services, session }: ActionCtx<PlayInput>) => {
  ...
  const played = await services.waves.play({ id: input.wave, blip: input.blip, block: input.block, who: session.name, cell: input.cell });
  return { played };
});

export const reset = action(async ({ input, services, session }: ActionCtx<ResetInput>) => {
  if (!session.name) fail("invalid", "name yourself before playing");
  const played = await services.waves.play({ id: input.wave, blip: input.blip, block: input.block, who: session.name });
  return { played };
});

/** An answer to a vote gadget or the answer already given to take it back. */
export const vote = action(async ({ input, services, session }: ActionCtx<VoteInput>) => {
...
export const amend = action(async ({ input, services, session }: ActionCtx<AmendInput>) => {
  ...
  const amended = await services.waves.editBlip({ id: input.wave, blip: input.blip, block: input.block, who: session.name, body: input.body });
  return { amended };
});
