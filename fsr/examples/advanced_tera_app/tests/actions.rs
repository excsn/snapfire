...
fn ctx(app: &advanced_tera_app::AppCore) -> RequestCtx {
  RequestCtx { services: app.services().bind_anonymous(), ..Default::default() }
}
...
fn action_failures_are_typed() {
  ...
  let missing = block_on(call_action(&app, "nope", ctx(&app), Value::Map(ValueMap::new()))).unwrap_err();
  assert_eq!(missing.kind, FailureKind::NotFound);

  let invalid = block_on(call_action(&app, "add_server", ctx(&app), Value::Map(ValueMap::new()))).unwrap_err();
