    ...
    <article className="page tool">
      ...
      <loan-planner deposit={tool.deposit}>
        <template shadowrootmode="open">
          <style>{":host { display: block; margin: 20px 0; padding: 14px 16px; border: 1px solid #e3e5ea; border-radius: 8px; background: #fff; } label { display: flex; align-items: center; gap: 10px; } input { flex: 1; } output { min-width: 5em; font-weight: 600; } p { margin: 8px 0 0; color: #6b7280; font-size: 13px; }"}</style>
          <label>
            Borrow for
          ...
          <p>
            Back <b data-back>in {tool.days} days</b>, £{tool.deposit} held until then.
          </p>
        </template>
      </loan-planner>
