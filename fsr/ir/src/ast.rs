...
pub enum Expr {
  ...
  /// where `Path` is not.
  Host,
  /// `document.origin`, the scheme and host this deployment is reached over,
  /// `null` when it names none. One value for the whole deployment, so this
  /// is constant the way `Config` is rather than per-request like `Host`.
  Origin,
