...
* `hydrated_by` is what mounts the component over the server's markup: `Some(HydratedBy::React)` for a component with state or handlers or one rendering such a component inline, `Some(HydratedBy::ReactTree)` for a layout the React adapter mounts as one root with its page, `Some(HydratedBy::Vue)` for a single-file component the build lowered, `None` for a template nothing mounts. The plan marks `None` as `(static)`, `ReactTree` as `(tree)` (the word `HydratedBy::TREE`) and `Vue` as `(vue)` (`HydratedBy::VUE`); the JSON form writes them as `"hydrate": false`, `"hydrate": "tree"` and `"hydrate": "vue"`, with `React` alone left out. The renderer takes its markup rules from it, `ReactTree` under React's; see [Frameworks](#frameworks).
...
* `pub enum HydratedBy { React, ReactTree, Vue }`: a template lowers from TSX and TSX that needs the browser runs as React; a layout declared `tree(Layout)` is `ReactTree`; a `.vue` file the build lowered is `Vue`. Derives `Debug`, `Clone`, `Copy`, `PartialEq`, `Eq`. `HydratedBy::TREE` and `HydratedBy::VUE` are the words the plan and the JSON form use.
